...
class QwenAPIAgent(BaseAgent):
    def __init__(self, search_pipeline: UnifiedSearchPipeline):
        ...
        self.system_prompt = (
            ...
            "- If image/text is unclear → 'I can't determine from the visual information'"
            "- If beyond knowledge → 'This is beyond my current knowledge'"
    ...
    def _generate_single_response(
        ...
        search_results = self._retrieve_knowledge(query,image)
    ...
    def _retrieve_knowledge(
        ...
        try:
            ...
            if query and self.search_pipeline.web_search:
                raw_text_results = self.search_pipeline(query, k=MAX_RESULTS*2)  # Get more for filtering
            ...
            if image and self.search_pipeline.image_collection:
                raw_image_results = self.search_pipeline(image, k=MAX_RESULTS*2)
                # Filter by score and sort descending
