...
在CRAG-MM数据集上，该脚本生成响应（通过OpenAI API使用可选的语义评估模型进行评价），计算多轮对话的指标，并可选地保存结果
"""
...
DEFAULT_EVAL_MODEL = "deepseek-v3"
MAX_API_RETRIES = 3
...
class CRAGEvaluator:
    ...
    def __init__(
        ...
        #在这个代码中分词器用于处理文本输入，确保生成的响应不会超过最大长度限制。
    ...
    # "Your task is to determine if a prediction correctly answers a question based on the ground truth.\n\n"
    # "Rules:\n"
    # "1. Focus on the core answer: The prediction is correct if it provides the same final answer (e.g., yes/no) as the ground truth and captures the essential reason for that answer, even if the reason is phrased differently or derived from context (like an image).\n"
    # "2. Allow for contextual variations: The prediction can include additional details (e.g., from image context) or practical advice as long as it does not contradict the ground truth and the core meaning aligns.\n"
    # "3. Be lenient with reasoning: If the ground truth and prediction give different reasons but both lead to the same correct conclusion, the prediction is correct. Only mark it incorrect if the reasoning introduces factual errors or misleads.\n"
    # "4. Omission tolerance: The prediction does not need to include every detail from the ground truth, only those critical to the question. Minor omissions are acceptable if they don't change the answer's correctness.\n"
    # "5. Output a JSON object with a single field 'accuracy' whose value is true or false."
    def attempt_api_call(
        ...
        尝试使用OpenAI API进行结构化输出调用，重试次数为MAX_API_RETRIES。
        Args:
            client: OpenAI客户端实例。
            model_name: 要查询的模型（例如“gpt-4o-mini”）。
            messages: 对话消息对象的列表。
            max_retries: 最大重试次数。

        Returns:
            如果成功，则返回CRAGTurnEvaluationResult对象；如果所有尝试都失败，则返回None。
        """
    ...
    def evaluate_response(self, crag_turn_data: dict[str, any]) -> dict[str, any]:
        ...
        if not is_idk and not is_exact_match and self.eval_model_name:
            ...
            api_response = self.attempt_api_call(local_client, self.eval_model_name, messages)
            if api_response:
    ...
    def initialize_evaluation(self) -> None:
        ...
        self.agent_response_map = {} #维护每个interaction_id对应的agent_response
        self.all_turn_data = []
    ...
    def generate_agent_responses(self, progress_callback: Callable[[int, int], None] = None) -> None:
        ...
        for batch_idx, batch in enumerate(tqdm.tqdm(self.batch_iterator, desc="Generating responses", disable=not self.show_progress)):
            ...
            for conversation_history in conversation_histories:
                message_history = []
                interaction_id_history = [] #维护每个对话的interaction_id历史
    ...
    def evaluate_agent(self) -> tuple[dict[str, any], dict[str, any]]:
        """
        评估agent的响应，返回turn_evaluation_results和score_dictionaries
        """
    ...
    def truncate_agent_responses(self, agent_responses: list[str]) -> list[str]:
        ...
        这段代码是为了将每个agent的响应截断到最大允许长度75，如果超过了75，则会被截断掉。
        """
...
def main() -> None:
    ...
    )#validation 是验证集，public_test 是公开测试集
    parser.add_argument(
        "--num-conversations",
        type=int,
        default=-1,
    ...
    )#-1表示全部验证集，正数表示验证集的前n个对话
    parser.add_argument(
        "--suppress-web-search-api",
        action="store_true",
        help="Suppress web search API when calling the agent"
    )#这个参数用于抑制网络搜索API的调用，通常在任务1（单源增强）中使用，当参数设置为True时，代理将不会调用网络搜索API，而是直接使用验证集中的数据进行评估
    #action="store_true" 表示如果提供了这个参数，则将其值设置为True，否则为False
    parser.add_argument(
    ...
    )#这个参数用于指定语义评估模型，通常在任务2（多源增强）中使用，可以用OpenAI的模型进行语义评估，也可以用其他模型，比如BERT等
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./",
        help="Path to save turn evaluation results and scores dictionary",
    )#这个参数用于指定保存结果的路径，如果不指定，则不会保存结果
    parser.add_argument(
        "--no_progress",
        action="store_true",
    ...
    available_splits = list(dataset.keys())#keys() 返回一个包含数据集可用分割的列表，例如 ['train', 'validation', 'public_test']。
    split_to_use = args.split if args.split in available_splits else available_splits[0]#如果指定的分割不存在，则使用第一个分割
    console.print(
    ...
    )#UnifiedSearchPipeline是用来统一管理搜索API的类，包括文本搜索、图像搜索、网页搜索等。
